...
.seller-info {
  ...
  background: $mtGrey;
  padding-top: 0.1rem;
  .container { display: flex; align-items: center; background: #fff; border-bottom: 1px solid $mtGrey; padding: 0 0.1rem; }
  .address { display: flex; align-items: center; @include px2rem(height, 100); }
  .icon { font-size: 0.4rem; }
  .describe { flex: 1; color: #222; font-size: 0.4rem; margin: 0 0.1rem; }
  .safe-file, .delivery-info, .delivery-time, .notification, .poi-service { @include px2rem(height, 80); }
  .notification { .ellipsis { @include ellipsis; } }
  .poi-service {
    .service { color: #222; margin: 0 0.1rem; }
    .icon-service { display: inline-block; @include px2rem(width, 30); @include px2rem(height, 30); margin: 0 0.5rem; background-size: cover; vertical-align: middle; }
    span { font-size: 0.4rem; vertical-align: middle; }
  }
  .safe-file, .delivery-time { margin-bottom: 0.2rem; }
  .discount {
    ul {
      background: #fff;
      li {
        display: flex; align-items: center; @include px2rem(line-height, 60);
        .active-icon { display: inline-block; @include px2rem(width, 30); @include px2rem(height, 30); background-size: cover; margin: 0 0.1rem; }
