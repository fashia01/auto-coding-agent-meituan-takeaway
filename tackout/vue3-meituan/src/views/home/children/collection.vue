        ...
        <div class="main-container">
          ...
          <div class="info-container">
            ...
            <div class="bottom margin-2">
              <span class="delete" @click.stop="deleteCollectionItem(item.id, index)"><i class="iconfont">&#xe615;</i> 取消收藏</span>
            </div>
          </div>
        </div>
      </article>
...
function deleteCollectionItem(id, index) {
  deleteCollectionApi({ id }).then((response) => {
    const res = response.data
    if (res.status === 200) {
      alertText.value = '取消收藏成功'
      showTip.value = true
      collectionList.value.splice(index, 1)
    } else {
      alertText.value = res.message || '取消收藏失败'
      showTip.value = true
    }
  })
}
...
onMounted(() => {
  ...
  getCollection().then((response) => {
    const res = response.data
    if (res.status === 200) collectionList.value = res.data || []
  })
})
</script>
...
#collection {
  ...
  .userinfo { margin-top: 1rem; text-align: center; .avatar { img { @include px2rem(width, 150); @include px2rem(height, 150); border-radius: 50%; } } .username { font-size: 0.5rem; } .collection-count { font-size: 0.35rem; } }
  article {
    padding: 0.3rem;
