  ...
  <div id="order-detail">
    ...
    <section class="tip">
      ...
      <div class="button-group">
        ...
        <router-link v-if="orderData.code === 200 && !orderData.has_comment" class="buy-again comment-btn" :to="{ path: '/make_comment', query: { order_id: orderData.id } }" tag="span">去评价</router-link>
      </div>
    </section>
...
const foods = ref([])
const orderData = ref({ total_price: 0 })
const address = ref({})
const alertText = ref('')
...
onMounted(() => {
  ...
  orderInfo({ order_id: id }).then((response) => {
    ...
    if (data.code === 200) {
      orderStatus.value = '订单已完成'
      statusDesc.value = '感谢您对美团外卖的支持，欢迎再次光临'
    } else {
      orderStatus.value = '订单已取消'
      statusDesc.value = '支付超时，订单已取消'
    }
...
#order-detail {
  ...
  background: #f4f4f4;
  .tip {
    margin-top: 0.4rem; text-align: center; background: #fff;
    h3 { padding: 0.4rem 0; font-size: 0.5rem; font-weight: 600; }
    h4 { padding: 0.2rem 0; color: $shallow_grey; font-size: 0.4rem; }
    .button-group { display: flex; justify-content: center; gap: 0.3rem; padding: 0.4rem 0; }
    .buy-again { margin: 0; display: inline-block; text-align: center; font-size: 0.35rem; background: #ffd161; @include px2rem(width, 190); @include px2rem(line-height, 70); &.comment-btn { background: #4aaaff; color: #fff; } }
  }
  .foods-info-container {
    ...
    .foods-container {
      display: flex;
      .foods-picture { display: inline-block; margin-right: 0.3rem; @include px2rem(width, 110); @include px2rem(height, 110); img { width: 100%; height: 100%; } }
      .main-container { flex: 1; div:first-child { display: flex; font-size: 0.4rem; .foods-name { flex: 1; } } & > span { margin: 0.2rem 0; display: block; font-size: 0.4rem; color: $shallow_grey; } }
    }
