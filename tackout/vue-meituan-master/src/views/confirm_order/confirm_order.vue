  ...
  export default {
    ...
    computed: {
      ...
      estimatedArrivalTime() {
        if (!this.poi_info || !this.poi_info.delivery_time_tip) {
          return '立即配送';
        }
        // 从配送时间提示中提取分钟数，如 "50分钟" -> 50
        const deliveryTimeMatch = this.poi_info.delivery_time_tip.match(/(\d+)/);
        ...
        const formattedTime = `${hours.toString().padStart(2, '0')}：${minutes.toString().padStart(2, '0')}分到`;
